...
// validatePath performs comprehensive path validation including
// format checks, security validation, and path traversal prevention.
func (r *pathResolver) validatePath(path string) error {
	...
	// Validate path length (reasonable limit to prevent resource exhaustion)
	const maxPathLength = 4096
	if len(path) > maxPathLength {
		return fmt.Errorf("path too long: %d characters (max %d)", len(path), maxPathLength)
	}
	
	// Clean path to handle . and .. components
	cleanPath := filepath.Clean(path)
	
	// If path is relative, check against base directory
	if !filepath.IsAbs(cleanPath) && r.baseDir != "" {
		absolutePath := filepath.Join(r.baseDir, cleanPath)
		relPath, err := filepath.Rel(r.baseDir, absolutePath)
		if err != nil {
			return fmt.Errorf("failed to validate relative path: %w", err)
		}
...
func (r *pathResolver) isWithinBaseDir(path string) bool {
	if r.baseDir == "" {
	...
	// Path is within base directory if it doesn't start with ".."
